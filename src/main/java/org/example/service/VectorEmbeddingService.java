...
public class VectorEmbeddingService {
    ...
    public void init() {
        if (apiKey == null || apiKey.trim().isEmpty() || apiKey.equals("your-api-key-here")) {
            logger.error("Embedding API Key 未配置！请设置环境变量 DASHSCOPE_API_KEY");
            throw new IllegalStateException("请设置环境变量 DASHSCOPE_API_KEY 后重新启动");
        }

        // 设置全局 API Key（DashScope SDK 通过该静态字段取凭据）
        Constants.apiKey = apiKey;

        textEmbedding = new TextEmbedding();

        logger.info("DashScope Embedding 服务初始化完成，模型: {}", model);
    }
