...
public class AiOpsService {
    ...
    private static final String AI_OPS_TASK_PROMPT =
            "你是企业级 SRE，接到了自动化告警排查任务。请结合工具调用，执行**规划→执行→再规划**的闭环，"
                    + "并最终按照固定模板输出《告警分析报告》。禁止编造虚假数据，"
                    + "如连续多次查询失败需诚实反馈无法完成的原因。";
     ...
     * 执行 AI Ops 告警分析流程（流式）
     *
     * <p>刻意使用 {@code stream()} 而非 {@code invoke()}：网关对非流式请求约 60s 就会以
     * 504 截断（实测为空闲超时），而多 Agent 编排的单次模型调用常常超过这个窗口。
     * 流式下同样的请求可稳定跑完数百秒。</p>
     *
     * <p>终止的 {@code __END__} 节点携带完整 OverAllState，planner_plan 仍在其中，
     * 因此报告内容用 {@link #extractFinalReport} 从状态里取，而不是从 token 流拼接。</p>
     *
     * @param chatModel     大模型实例
     * @param toolCallbacks 工具回调数组
     * @return 图执行过程的节点/ token 输出流
     * @throws GraphRunnerException 如果 Agent 构建或启动失败
     */
    public Flux<NodeOutput> streamAiOpsAnalysis(ChatModel chatModel, ToolCallback[] toolCallbacks) throws GraphRunnerException {
        logger.info("开始执行 AI Ops 多 Agent 协作流程（流式编排）");

        SupervisorAgent supervisorAgent = buildSupervisorAgent(chatModel, toolCallbacks);
        return supervisorAgent.stream(AI_OPS_TASK_PROMPT);
    }

    /**
     * 构建 Supervisor Agent
     */
    private SupervisorAgent buildSupervisorAgent(ChatModel chatModel, ToolCallback[] toolCallbacks) {
