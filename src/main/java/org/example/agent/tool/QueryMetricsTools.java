...
public class QueryMetricsTools {
    ...
    public String queryPrometheusAlerts() {
        ...
        try {
            ...
            if (mockEnabled) {
                ...
                // 同一条告警规则在不同实例/挂载点上触发时，是彼此独立的现场，必须分别保留。
                // 之前只按 alertname 去重，会把 HighDiskUsage 的多个挂载点压成一条，
                // 丢掉"到底是哪几个盘在告警"这一关键信息。
                ...
                for (PrometheusAlert alert : result.getData().getAlerts()) {
                    Map<String, String> labels = alert.getLabels() == null
                            ? Collections.emptyMap() : alert.getLabels();
                    ...
                    String incidentKey = alertName + "|" + instance + "|" + mountpoint;
                    if (!seenIncidents.add(incidentKey)) {
                        continue;
                    }
                    ...
                    simplified.setMountpoint(mountpoint);
     ...
     *
     * <p>Prometheus 与应用通常不在同一台机器，activeAt 晚于本地时钟是常态；
     * 此处把负值收敛为 0，避免出现 "-2h3m" 这种会误导模型判断的时长。</p>
     */
    private String calculateDuration(String activeAtStr) {
        try {
            ...
            if (duration.isNegative()) {
                logger.warn("告警 activeAt 晚于本地时钟，按 0 处理: {}", activeAtStr);
     ...
     * Prometheus 告警信息结构
     *
     * <p>忽略未知字段：Prometheus 会跨版本给告警对象新增字段（如 url、generatorURL），
     * 而 ObjectMapper 默认对未知字段抛异常，会让整条告警查询直接失败。</p>
     */
    @Data
