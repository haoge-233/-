...
class SuperBizAgentApp {
    constructor() {
        // 用相对路径：原先硬编码 http://localhost:9900/api，
        // 一旦从别的机器访问（SERVER_ADDRESS=0.0.0.0）界面就会全部打不通
        this.apiBaseUrl = '/api';
        this.authRequired = false;
        this.tokenStorageKey = 'superbizagent.apiToken';
        ...
        // 异步探测服务端是否开启鉴权；在此之前发出的请求若被 401，apiFetch 会兜底提示输入
    ...
    /**
     * 读取服务端配置，获知是否开启了接口鉴权。
     * 读不到时按未开启处理：真正的未授权会由 401 分支兜底提示，不必在这里猜
     */
    async loadServerConfig() {
        try {
            ...
            if (res.ok) {
                ...
                this.authRequired = !!cfg.authRequired;
                if (cfg.tokenStorageKey) {
                    this.tokenStorageKey = cfg.tokenStorageKey;
                }
            }
        } catch (e) {
            console.warn('读取服务端配置失败，将按无鉴权模式运行', e);
        }
    }
    ...
    /** 提示用户输入令牌；返回 true 表示拿到了新令牌，可以重试 */
    promptForToken() {
        const token = window.prompt(
            '该服务已开启接口鉴权，请输入访问令牌（API Token）：',
            this.getApiToken()
        );
        if (!token) {
            return false;
        }
        sessionStorage.setItem(this.tokenStorageKey, token.trim());
        this.authRequired = true;
        return true;
    }

    /**
     * 统一的 API 请求入口：自动带上令牌，401 时提示重填并重试一次
     */
    async apiFetch(path, options = {}) {
        const send = () => fetch(`${this.apiBaseUrl}${path}`, {
            ...options,
            headers: this.authHeaders(options.headers || {})
        });

        let res = await send();
        if (res.status === 401 && this.promptForToken()) {
            res = await send();
        }
        return res;
    }
    ...
    async sendQuickMessage(message) {
        ...
        try {
            ...
            if (!response.ok) {
                // 服务端会带 JSON 正文说明具体原因（额度用尽 / 密钥无效 / 向量库不可用…），
                // 只报 HTTP 状态码会让用户完全不知道该做什么
                let reason = `HTTP错误: ${response.status}`;
                try {
    ...
    async uploadFile(file) {
        ...
        try {
            ...
            // 失败时服务端也会带上 JSON 正文（含已保存路径与失败原因），所以先解析再判断状态
            let data = null;
            try {
            ...
            const serverMessage = data && data.message ? data.message : `HTTP错误: ${response.status}`;

            if (!response.ok && data && data.data && data.data.indexed === false) {
                // 文件已落盘但没进向量库：必须说清楚，否则用户会以为已经可以检索
                this.addMessage('assistant',
                    `⚠️ ${file.name} 已保存到服务器，但向量化失败，知识库检索不到它。\n` +
                    `原因：${data.data.indexError || serverMessage}\n` +
                    `请修复后重新上传该文件。`, false, true);
                this.showNotification(`${file.name} 未成功入库（已保存但未向量化）`, 'error');
                return;
            }
            ...
            if (data && data.data && data.data.indexed && response.status === 200) {
                // 在聊天界面显示上传成功消息
