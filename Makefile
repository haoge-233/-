...
.PHONY: help init start stop restart check upload clean up down status wait obs-up obs-down obs-status
...
# 启动本地可观测栈（Prometheus + node_exporter），供 AIOps 读取真实告警
obs-up:
	@echo "$(YELLOW)启动 Prometheus 可观测栈...$(NC)"
	@docker compose -f observability.yml up -d
	@echo "$(YELLOW)等待告警规则评估...$(NC)"
	@sleep 25
	@echo "$(GREEN)Prometheus: http://localhost:9090  (告警接口 /api/v1/alerts)$(NC)"
	@echo "$(YELLOW)以真实模式启动服务: export PROMETHEUS_MOCK_ENABLED=false$(NC)"

# 停止本地可观测栈
obs-down:
	@echo "$(YELLOW)停止 Prometheus 可观测栈...$(NC)"
	@docker compose -f observability.yml down
	@echo "$(GREEN)已停止$(NC)"

# 列出当前 firing 的告警
obs-status:
	@curl -s --max-time 10 http://localhost:9090/api/v1/alerts | python -c "import json,sys;a=json.load(sys.stdin)['data']['alerts'];print('firing:',len(a));[print(' ',x['labels']['alertname'],'|',x['labels'].get('instance',''),'|',x['labels'].get('mountpoint','')) for x in a]"
